...
$sql = "
    ...
    WHERE r.vendor_id = '$vendor_id'
      AND DATE(b.created_at) BETWEEN '$from_date' AND '$to_date'
      $statusSql
...
if (!$result) {
    die("Failed to fetch vendor bookings");
}
...
if (count($rows) === 0) {
    $html .= '
      <tr>
        <td colspan="12">No bookings found for selected filters.</td>
      </tr>
    ';
} else {
    foreach ($rows as $row) {
...
$filename = 'vendor_bookings_report_' . date('Ymd_His') . '.pdf';
