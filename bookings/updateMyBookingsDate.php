...
if (date('Y-m-d') >= $booking['check_in']) {
    echo json_encode([
        "success" => false,
        "message" => "Booking dates cannot be modified on or after check-in date"
    ]);
    exit;
}
...
/* overlap check against other bookings */
$sqlOverlap = "
    SELECT booking_id
    FROM bookings
...
$new_total_price = $nights * $room_price;
